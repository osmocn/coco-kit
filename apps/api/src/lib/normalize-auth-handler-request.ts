...
function normalizeRedirectFields(
  ...
  for (const fieldName of redirectFieldNames) {
    ...
    payload[fieldName] = normalizeTrustedCallbackURL(value, headers);
    changed = true;
  }
...
export async function normalizeAuthHandlerRequest(request: Request) {
  ...
  for (const fieldName of redirectFieldNames) {
    ...
    url.searchParams.set(fieldName, normalizeTrustedCallbackURL(value, headers));
    queryChanged = true;
  }
  ...
  if (contentType.includes("application/json")) {
    ...
    const bodyChanged = normalizeRedirectFields(nextPayload, headers);
  ...
  if (contentType.includes("application/x-www-form-urlencoded")) {
    ...
    for (const fieldName of redirectFieldNames) {
      ...
      payload.set(fieldName, normalizeTrustedCallbackURL(value, headers));
      bodyChanged = true;
    }
